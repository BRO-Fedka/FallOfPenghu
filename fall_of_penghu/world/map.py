...
def load_map(map_dir: Path) -> MapData:
    ...
    data = MapData(manifest=manifest, map_dir=map_dir)

    for feat in _load_collection(map_dir / "taiwan.geojson"):
        data.taiwan.extend(_polys_from_feature(feat))
    ...
    for feat in _load_collection(map_dir / "airports.geojson"):
        ...
        if gtype in ("LineString", "MultiLineString"):
            data.airport_lines.extend(
                _lines_from_feature(feat, highway=kind, width_m=12.0)
            )
        else:
            data.airports.extend(
                _polys_from_feature(feat, icao=extra["icao"], apt_kind=kind)
            )

    data.coast_grid = _index(data.coast, 2_000.0)
    data.veg_grid = _index(data.vegetation, 2_000.0)
    data.building_grid = _index(data.buildings, 500.0)
    data.road_grid = _index(data.roads, 1_000.0)
    return data
